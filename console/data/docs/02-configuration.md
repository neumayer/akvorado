...
- `topic` is the destination topic. Unlike the inlet topic, the outlet does not
  yet manage this topic, so create it — with retention — beforehand.
- `topic-schema-suffix` (default: `true`) appends the schema hash to the topic
  name (`topic-<hash>`) — the same hash embedded in the generated message name
  (`FlowMessagev<hash>`). An incompatible schema change then lands on a new topic
  instead of mixing wire layouts for consumers, mirroring the inlet topic's
  version suffix. Set it to `false` to keep the topic name verbatim.
