...
//
// The topic is the configured name suffixed with the schema hash by default
// (disable with topic-schema-suffix), so an incompatible schema change lands on
// a new topic instead of mixing wire layouts for consumers. The outlet does not
// yet manage the topic the way the orchestrator manages the inlet topic, so
// creation and retention are configured at deploy time. Consumers should track
// the schema's Protobuf definition (Schema.ProtobufDefinition) for the layout.
package kafkaout
...
func New(r *reporter.Reporter, configuration Configuration, dependencies Dependencies) (*Component, error) {
	...
	if configuration.TopicSchemaSuffix {
