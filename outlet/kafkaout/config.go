...
type Configuration struct {
	...
	// TopicSchemaSuffix appends the protobuf schema hash to the topic name
	// (topic-<hash>), so an incompatible schema change lands on a new topic
	// rather than mixing wire layouts for consumers. The hash matches the one in
	// the generated message name (FlowMessagev<hash>). On by default; set false
	// to keep the topic name verbatim.
	TopicSchemaSuffix bool
}
